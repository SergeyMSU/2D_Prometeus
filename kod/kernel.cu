...
cudaError_t addWithCuda_M_K(Konstruktor& K);
...
int main()
{
    ...
    if (false)
    {
        cudaError_t cudaStatus = addWithCuda_M_K(K);
...
cudaError_t addWithCuda_M_K(Konstruktor& K)
{
    ...
    double* host_x, * host_y;
    double* host_ro1, * host_p1, * host_u1, * host_v1;
    double* host_I_u, * host_I_v, * host_I_T;
    double* dev_I_u, * dev_I_v, * dev_I_T;
    int* host_l, * host_r;
    int* dev_size, * host_size;
    int met = 0;
    ...
    host_l = new int[N];
    host_r = new int[N];
    host_I_u = new double[N];
    host_I_v = new double[N];
    host_I_T = new double[N];
    ...
    *Nu = N;
    ...
    for (int i = 0; i < K.all_Kyb.size(); i++)
    {
        ...
        host_u1[i] = K.all_Kyb[i]->u;
        host_v1[i] = K.all_Kyb[i]->v;
        host_I_u[i] = K.all_Kyb[i]->I_u;
        host_I_v[i] = K.all_Kyb[i]->I_v;
        host_I_T[i] = K.all_Kyb[i]->I_T;
        host_l[i] = ll;
        host_r[i] = ll + K.all_Kyb[i]->sosed.size() - 1;
        ll = ll + K.all_Kyb[i]->sosed.size();
    }
    ...
    if (true)
    {
        ...
        cudaStatus = cudaMalloc((void**)&dev_I_u, N * sizeof(double));
        cudaStatus = cudaMalloc((void**)&dev_I_v, N * sizeof(double));
        cudaStatus = cudaMalloc((void**)&dev_I_T, N * sizeof(double));
    ...
    if (true)
    {
        ...
        cudaStatus = cudaMemcpy(dev_I_u, host_I_u, N * sizeof(double), cudaMemcpyHostToDevice);
        cudaStatus = cudaMemcpy(dev_I_v, host_I_v, N * sizeof(double), cudaMemcpyHostToDevice);
        cudaStatus = cudaMemcpy(dev_I_T, host_I_T, N * sizeof(double), cudaMemcpyHostToDevice);
    }
    ...
    for (int i = 0; i < 100000; i = i + 2)  // Сколько шагов по времени делаем?
    {
        ...
        // запускаем add() kernel на GPU, передавая параметры
        Cuda_main_HLLDQ_M_K << <NNN, 256 >> > (dev_N, dev_x, dev_y, dev_size,//
            dev_ro1, dev_ro2, dev_Q1, dev_Q2, dev_p1, dev_p2, dev_u1, dev_u2, dev_v1, dev_v2, dev_I_u, dev_I_v, dev_I_T,//
            dev_sosed, dev_l, dev_r, dev_T, dev_T_do, i, K.DX, K.DY, met);
        ...
        Cuda_main_HLLDQ_M_K << <NNN, 256 >> > (dev_N, dev_x, dev_y, dev_size,//
            dev_ro2, dev_ro1, dev_Q2, dev_Q1, dev_p2, dev_p1, dev_u2, dev_u1, dev_v2, dev_v1, dev_I_u, dev_I_v, dev_I_T,//
            dev_sosed, dev_l, dev_r, dev_T, dev_T_do, i, K.DX, K.DY, met);
    ...
    K.read_Cuda_massiv(host_ro1, host_p1, host_u1, host_v1, host_Q1);
    K.print_Tecplot();
    K.Save_setka_MK("all_save_3.txt");
